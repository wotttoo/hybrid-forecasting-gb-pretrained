...
def read_and_melt_m4(file_path, max_series=None, series_ids=None):
    """
    ...
    max_series : int, optional
        Giới hạn số dòng đọc từ đầu file — chỉ nên dùng cho việc test
        nhanh code, KHÔNG dùng để chọn tập dữ liệu chính thức.
    series_ids : list, optional
        Danh sách unique_id cụ thể cần giữ lại (dùng cho tập đã qua
        stratified sampling từ src/length_grouping.py).
    """
    ...
    # Lọc đúng series đã chọn từ bước sampling, nếu có
    if series_ids is not None:
        df_wide = df_wide[df_wide['unique_id'].isin(series_ids)]

    df_long = pd.melt(
        df_wide, id_vars=['unique_id'],
        var_name='time_step', value_name='y'
    ).dropna(subset=['y'])
...
def load_and_preprocess_m4_monthly(train_path, test_path, max_series=None, series_ids=None):
    print("--- Bước 1 & 2: Đọc và Melt dữ liệu ---")
    train_long = read_and_melt_m4(train_path, max_series=max_series, series_ids=series_ids)
    test_long = read_and_melt_m4(test_path, max_series=max_series, series_ids=series_ids)
